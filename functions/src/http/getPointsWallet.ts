/**
 * getPointsWallet.ts — Returns the full wallet state for the Points Wallet screen.
 *
 * Authenticated callable. Returns:
 *   currentBalance, totalEarned, totalSpent, tier, tierProgress, expiringEntries,
 *   recentHistory (last 10 entries)
 *
 * Milestone: B5
 */
...
export const getPointsWallet = onCall(
  { region: "us-central1" },
  async (request) => {
    ...
    log.info("getPointsWallet: start", {
    ...
    // 2. Compute rolling 12-month earned points
    ...
    // 4. Get expiring entries (within 60 days)
    ...
    // 5. Get recent history (last 10 entries)
    ...
    log.info("getPointsWallet: complete", {
      traceId, userId: uid, domain: "points", eventId: `wallet_${uid}`,
    }, { tier, rolling12MonthPoints });
